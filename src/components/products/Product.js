...
const Product = (props) => {
    ...
    const { name, image, short_description, price, rating } = props;
    ...
    return (
        <div className="flex flex-col justify-center items-center space-y-3 bg-white border border-gray-200 hover:shadow-xl transition duration-700 ease-in-out transform hover:scale-105 p-4 box-border rounded-xl">
            ...
            <div className="flex items-center space-x-3">
                {user && user.displayName ? ( // Перевірка на існування user
                    <>
                        <button 
                            disabled={disabled} 
                            className={`${disabled ? "opacity-30" : ""} w-36 btn-primary py-3 px-4 poppins text-sm flex items-center justify-center space-x-3 text-center`} 
                            onClick={() => {
                                handleCart(props);
                                setDisabled(true);
                        ...
                        >
                            <BsCart2 />
                            <span>{orders.find(item => item.id_products === props.id_products) || disabled ? "Added" : "Add To Cart"}</span>
                        </button>
                    </>
                ) : null} 
               
                <Button className="w-36 btn-primary py-3 px-2 poppins text-sm" text="View" onClick={() => history.push(`/products/${name}`)} />
            </div>
        </div>
    )
}
